...
class ViewController: UIViewController {
    ...
    func loadingIndicator(isActive loading: Bool) {
        if loading  {
...
extension ViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.orgsList[section]?.branches?.count ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        return "\(viewModel.orgsList[section]?.name ?? "Unknown name") [\(viewModel.orgsList[section]?.language ?? "Unknown language")]"
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = viewModel.orgsList[indexPath.section]?.branches?[indexPath.row].name
        return cell
    }
